...
void *ballsinboxes(void *dim) {
	...
	unsigned int max = ((struct targ *)dim)->max;
...
INT **threadedbinb(INT n, INT m, mpfr_t *xi, unsigned int numThreads, gsl_rng **rgens, unsigned int num) {
	...
	mpfr_set_d(norm, 1.0, MPFR_RNDN);

	// q[i] = xi[i] / (xi[i] + ... + xi[n-1])
	//      = xi[i] / (norm - xi[0] - ... xi[i-1])
	mpfr_set_d(sumXi, 0.0, MPFR_RNDN);
	for(i=0; i<n; i++) {
...
/*
 * Like threadedbin, but with precomputed weights
 */
INT **threadedbinb_prec(INT n, INT m, double *q, unsigned int max, unsigned int numThreads, gsl_rng **rgens, unsigned int num) {
	struct targ *argList;   // arguments for the separate threads
	pthread_t *th;			// array of threads
	INT i;
	void *ret;
...
/*
 * Like tbinb, but with precomputed weights
 */
INT *tbinb_prec(INT n, INT m, double *q, unsigned int max, unsigned int numThreads, gsl_rng **rgens) {
	INT **multi;
	INT *single;
