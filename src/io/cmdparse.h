...
struct cmdarg
{
	...
	INT size;					// target size of random graph
	...
	char *gcla;				// the graph class from which we want to sample
	int Tgcla;				// has value been set by the user?
};
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
	...
	const char *graphclasses[] = {"tree", "cacti", "outer", "series"};
	int num;
	...
	switch (key) {
		...
		case 'g':
			// select graph class
			for(num=0; num<4; num++) {
				if( strcmp(graphclasses[num],arg) == 0) {
...
*/
static char doc[] = "grogue -- Generate Random blOck-stable Graphs";
